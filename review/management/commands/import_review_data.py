...
# Create file handler
file_handler = logging.FileHandler('zlogs/import_review_data.log')
...
class Command(BaseCommand):
    help = 'Import review questions, answers, and explanations from JSON files'
    ...
    def handle(self, *args, **options):
            ...
            try:
                ...
                for quiz_question in question_list:
                    ...
                    for choice in quiz_question["choices"]:
                        ...
                        answer_msg = f"Answer: {answer.text} - {answer.is_correct}"
                        self.stdout.write(answer_msg)
                        logger.info(answer_msg)
                ...
                failed_imports += 1
                failed_categories.append(f"{category_exam_type} - {category_name}")
                continue
        ...
        summary_msg += f"\nTotal Questions Imported: {total_questions_imported}"
        
        if successful_imports > 0:
            summary_msg += f"\n\nSuccessfully Imported Categories:"
            for tup in import_summary:
                summary_msg += f"{tup[0]} - {tup[1]}, "
